...
int main () {
    ...
    bool digitsIndex = 0;
    int winnerIndex = -1;
    for (size_t i = 0; (boards.size() > 1 && i < digitsChosen.size()); i++){
    ...
    for (size_t i = digitsIndex; i < digitsChosen.size(); i++){
        newDigits.push_back(digitsChosen[i]);
        if(isWinner(boards[0],newDigits)){
            digitsIndex = i;
            break;
        }
    }

    std::cout << boards.size() << endl;
    std::cout << boards[0][0][0] << endl;
    int count = 0;
    for (size_t i = 0; i < boards[0].size(); i++){
        for (size_t j = 0; j < boards[0][i].size(); j++){
            if (std::find(newDigits.begin(), newDigits.end(), boards[0][i][j]) == newDigits.end()){
                count+=boards[0][i][j];
            }
        }
    }
